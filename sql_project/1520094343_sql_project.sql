...
SELECT
    CASE WHEN m.memid = 0 then guestcost 
		ELSE membercost END AS real_cost,
...
WHERE b.starttime LIKE '2012-09-14%'
    AND CASE WHEN m.memid = 0 THEN guestcost 
		ELSE membercost END > 30.0
ORDER BY  1 DESC
...
FROM(
SELECT CASE WHEN m.memid = 0 then guestcost 
		ELSE membercost END AS real_cost,
        CONCAT(m.firstname,' ',m.surname) AS member_name,
        f.name, b.starttime
    FROM country_club.Bookings b
...
WHERE sub.starttime LIKE '2012-09-14%'
    AND sub.real_cost > 30.0
ORDER BY sub.real_cost DESC
...
FROM(
SELECT f.name,
        SUM(CASE WHEN m.memid = 0 THEN guestcost 
		ELSE membercost END) AS real_cost
    FROM country_club.Bookings b
        INNER JOIN country_club.Members m ON b.memid = m.memid
        INNER JOIN country_club.Facilities f ON b.facid = f.facid
